...
async def execute_tool_call(tool_name: str, params: dict) -> dict:
    """Idea 04: VAPI action agent — Gemini 2.0 / GPT-4o tool_use does real bookings.
    ...
    if settings.GEMINI_API_KEY:
        try:
            ...
            prompt = f"Execute tool {tool_name} with parameters {params}. Respond with the result of this action as a JSON object."
            response = model.generate_content(prompt, generation_config={"response_mime_type": "application/json"})
            
            import json
            return {"tool": tool_name, "result": json.loads(response.text), "provider": "gemini", "status": "executed"}
        except Exception as e:
            logger.warning(f"Gemini tool execution failed: {e}. Falling back to OpenAI...")

    # 2. Try OpenAI Native SDK
        ...
        try:
            ...
            response = client.chat.completions.create(
                ...
                messages=[
                    ...
                    {"role": "user", "content": f"Execute tool {tool_name} with parameters {params}."}
                ],
            ...
            return {"tool": tool_name, "result": json.loads(result_content), "provider": "openai", "status": "executed"}
        except Exception as e:
            logger.warning(f"OpenAI tool execution failed: {e}")

    # 3. OpenRouter Fallback (Original)
        ...
        try:
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                if response.status_code == 200:
                    ...
                    return {"tool": tool_name, "result": json.loads(result_content), "provider": "openrouter", "status": "executed"}
        except Exception as e:
            logger.error(f"OpenRouter Tool execution failed: {e}")

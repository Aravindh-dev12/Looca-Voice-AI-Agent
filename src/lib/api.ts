...
async function apiFetch<T>(path: string, method: Method = 'GET', body?: any): Promise<T> {
  ...
  try {
    ...
    throw new Error(`Network error: Cannot connect to backend at ${API_BASE}. Is the server running?`);
  }
  ...
  if (!contentType || !contentType.includes('application/json')) {
    ...
    console.error(`Expected JSON from ${path} but got ${contentType || 'unknown'}:`, text.substring(0, 200));
    throw new Error(`Backend error: Expected JSON but received ${contentType || 'HTML'}. Is the backend server running at ${API_BASE}?`);
  }
...
export const api = {
  ...
  syncGoogle: (data: { name: string; email: string; image?: string }) =>
    apiFetch<{ access_token: string; user: any }>('/api/auth/google-sync', 'POST', data),
  ...
  voiceChat: (data: { message: string; language?: string; conversation_id?: string }) =>
    apiFetch<{ reply: string; conversation_id: string; action?: any; language_detected?: string }>('/api/voice/chat', 'POST', data),

  voiceChatTTS: (data: { text: string; voice?: string; language?: string }) => {
    const formData = new FormData();
    formData.append('text', data.text);
    formData.append('voice', data.voice || 'coral');
    formData.append('language', data.language || 'en');
    const token = typeof window !== 'undefined' ? localStorage.getItem('looca_token') : null;
    return fetch(`${API_BASE}/api/voice/chat/tts`, {
      method: 'POST',

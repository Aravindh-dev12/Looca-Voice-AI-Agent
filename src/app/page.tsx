...
export default function LandingPage() {
  ...
  useEffect(() => {
    ...
    if (checkStandalone && !user) {
      ...
      setTimeout(() => {
        setShowSplash(false);
        // Redirect to mobile voice assistant page for PWA users
  ...
  const handleDownload = async () => {
    // If PWA install prompt is available (Chrome/Edge on Android, desktop)
    if (deferredPrompt) {
      deferredPrompt.prompt();
      const { outcome } = await deferredPrompt.userChoice;
      if (outcome === 'accepted') {
        setDeferredPrompt(null);
        // After install, prompt to save Looca as contact
        saveLoocaContact();
      }
      return;
    }

    // iOS Safari doesn't support beforeinstallprompt — show hint
    ...
    // Fallback: open auth modal (user signs up first, then can install)
    setAuthOpen(true);
  };

  const saveLoocaContact = () => {
    // Download vCard so Looca appears in phone contacts
    const link = document.createElement('a');
    link.href = '/looca-contact.vcf';
    link.download = 'Looca-AI.vcf';
          ...
          <div className="text-center max-w-4xl mx-auto">
            ...
            >
              ...
              <Button
                size="lg"
                variant="outline"
                className="h-14 px-10 text-lg rounded-full border-2 border-black hover:bg-black hover:text-white transition-all shadow-xl group"
              ...
              {showIOSHint && (
                <motion.div
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  exit={{ opacity: 0 }}
                  className="absolute -bottom-16 left-1/2 -translate-x-1/2 bg-white border border-gray-200 shadow-xl rounded-xl px-5 py-3 text-sm text-gray-700 whitespace-nowrap z-10"
                >
                  Tap <span className="inline-block mx-1"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="inline"><path d="M12 5v14M5 12l7-7 7 7"/></svg></span> then "Add to Home Screen"
                </motion.div>
              )}
            </motion.div>
        ...
        <div className="relative max-w-4xl mx-auto px-4 text-center">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            ...
            <Button
              size="lg"
              variant="outline"
              className="h-16 px-12 text-xl rounded-full border-2 border-black hover:bg-black hover:text-white transition-all shadow-2xl group"
              onClick={handleDownload}
            >

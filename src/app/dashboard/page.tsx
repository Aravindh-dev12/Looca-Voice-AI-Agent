...
export default function TalkDashboard() {
  ...
  const processVoiceCommand = async (command: string) => {
    ...
    try {
      ...
      setStatus('waiting');
      setLiveLog(prev => [...prev, { type: 'result', text: `Looca: ${geminiReply}` }]);
    } catch (err) {
                ...
                <Card className="p-4 border-slate-200 shadow-sm bg-white">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <p className="text-[10px] font-bold text-slate-400 uppercase tracking-tighter">Language</p>
                    </div>
                    <Badge variant="outline" className="text-[10px] capitalize">
                      {intelligence?.language_detected || 'Detecting...'}
                    </Badge>
                  </div>
                </Card>
              </div>

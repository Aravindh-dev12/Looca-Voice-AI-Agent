...
// Icon mapping for sound effect categories
const iconMap = {
  Dog, Volume2, Zap, Music, Speaker
};
            ...
            {slug === 'tts' ? (
              ...
              <div className="flex flex-col h-full space-y-6">
                ...
                <div className="grid grid-cols-2 gap-3 md:grid-cols-4 xl:grid-cols-5">
                  {[
                    { label: 'Animals', icon: 'Dog', tint: 'from-emerald-400 to-emerald-700' },
                    { label: 'Bass', icon: 'Volume2', tint: 'from-indigo-400 to-indigo-700' },
                    { label: 'Booms', icon: 'Zap', tint: 'from-rose-400 to-rose-700' },
                    { label: 'Braams', icon: 'Music', tint: 'from-amber-400 to-amber-700' },
                    { label: 'Brass', icon: 'Speaker', tint: 'from-orange-400 to-orange-700' },
                  ].map((cat) => {
                    const IconComponent = { Dog, Volume2, Zap, Music, Speaker }[cat.icon as keyof typeof iconMap] || Music;
                    return (
                    <div key={cat.label} className="relative aspect-[0.9] overflow-hidden rounded-[20px] border border-zinc-200 bg-white shadow-sm transition-all hover:-translate-y-0.5 hover:shadow-md cursor-pointer" onClick={() => { setVfxPrompt(`Generate ${cat.label.toLowerCase()} sound effect`); startProcessing(); }}>
